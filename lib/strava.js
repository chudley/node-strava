...
Strava.prototype.listAthleteActivities = function listAthleteActivities(callback) {
	var self = this;
...
Strava.prototype.getActivity = function getActivity(id, callback) {
	mod_assertplus.number(id, 'id');
	mod_assertplus.func(callback, 'callback');

	var self = this;

	var path = sprintf('activities/%d', id);
	self._get(path, callback);
};
